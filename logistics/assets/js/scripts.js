...
var frmRequestQuoteExists = document.getElementById("frmrequestquote");
if (frmRequestQuoteExists) {
  document.getElementById("frmrequestquote").onsubmit = function (e) {
    e.preventDefault();
    var xhr = new XMLHttpRequest();
    var url = "functions/requestquote.php";
    xhr.open("post", url, true);
    var formData = new FormData(frmrequestquote);
    xhr.send(formData);
    xhr.onload = function () {
...
function submitRequestQuote() {
  ...
  if (document.getElementById("name").value == "") {
    ...
    document.getElementById("errmsg").innerHTML = "Phone No is mandatory";
    return false;
  } else if (document.getElementById("moving_from_city").value == "") {
    document.getElementById("errmsg").innerHTML =
      "Moving From City is mandatory";
    return false;
  } else {
    if (document.getElementById("moving_to_city").value == "") {
      document.getElementById("errmsg").innerHTML =
        "Moving To City is mandatory";
      return false;
    }
  }
...
var frmGetFreeQuoteExists = document.getElementById("frmgetfreequote");
if (frmGetFreeQuoteExists) {
  document.getElementById("frmgetfreequote").onsubmit = function (e) {
    //	document.getElementById("loading").style.display = "block";
    e.preventDefault();

    const path = window.location.pathname;
    currentPageUrl = path.substring(path.lastIndexOf("/") + 1);
    var xhr = new XMLHttpRequest();
    if (currentPageUrl == "ksa.php") {
      var url = "../functions/getfreequote.php";
    } else {
      var url = "functions/getfreequote.php";
    }
    xhr.open("post", url, true);
    var formData = new FormData(frmgetfreequote);
    xhr.send(formData);
    xhr.onload = function () {
      if (this.response == "Message has been sent") {
...
function submitRequest() {
  ...
  if (document.getElementById("name").value == "") {
    ...
    document.getElementById("errmsg").innerHTML = "Email is mandatory";
    return false;
  } else if (document.getElementById("moving_from_country").value == "") {
    document.getElementById("errmsg").innerHTML =
      "Moving From Country is mandatory";
    return false;
  } else {
    if (document.getElementById("moving_to_country").value == "") {
      document.getElementById("errmsg").innerHTML =
        "Moving To Country is mandatory";
      return false;
    }
  }
...
var frmmovingtoukExists = document.getElementById("frmmovingtouk");
if (frmmovingtoukExists) {
  document.getElementById("frmmovingtouk").onsubmit = function (e) {
	let button = document.querySelector(".iti__selected-country"); 
	var countrydet;
	var countryname;
	var country_code;
	if (button)
	{
		countrydet=button.getAttribute("title"); // OR console.log(button.title);
		let arr = countrydet.split(":");
		countryname=arr[0];
		country_code=arr[1];
	}
    ...
    currentPageUrl = path.substring(path.lastIndexOf("/") + 1);
    var xhr = new XMLHttpRequest();
    var url = "functions/moving-to-uk.php";
    xhr.open("post", url, true);
    var formData = new FormData(frmmovingtouk);
...
function submitMovingtouk() {
  ...
  {
		document.getElementById("errmsg").innerHTML = "Country Code is mandatory";
   		return false;
   }
    ...
    document.getElementById("errmsg").innerHTML =
      "Moving From Country is mandatory";
    return false;
  } else if (document.getElementById("moving_to_country").value == "") {
    document.getElementById("errmsg").innerHTML =
      "Moving To Country is mandatory";
    return false;
  } else {
...
var frminternationalmoversExists = document.getElementById("frminternationalmovers");
if (frminternationalmoversExists) {
  document.getElementById("frminternationalmovers").onsubmit = function (e) {
	let button = document.querySelector(".iti__selected-country"); 
	var countrydet;
	var countryname;
	var country_code;
    ...
    currentPageUrl = path.substring(path.lastIndexOf("/") + 1);
    var xhr = new XMLHttpRequest();
    var url = "functions/international-movers.php";
    xhr.open("post", url, true);
    var formData = new FormData(frminternationalmovers);
	formData.append("country", countryname);
	formData.append("country_code", country_code);
	formData.append("phoneno", phoneno);
    xhr.send(formData);
...
function submitInternationalmovers() {
  document.getElementById("errmsg").innerHTML = "";
  document.getElementById("loading").style.display = "none";
  document.getElementById("msgstatus").style.display = "none";
  let button = document.querySelector(".iti__selected-country"); 
...
let img = document.querySelector(".site-header__logo-img");
img.setAttribute("alt", "Alim Logo");
...
class MainNavigation {
  ...
  submenuDetect() {
    ...
    li.forEach((item) => {
      ...
      if (ul) {
        let submenu = item.classList.add("has-submenu");
  ...
  firstLevelMenuToggle() {
    let firstLevelMenu = this.mainContent
      .querySelector("ul")
      .querySelectorAll(":scope > li.has-submenu");
    firstLevelMenu.forEach((item) => {
      let submenuTrigger = item.querySelector(".down-arrow-icon");
      submenuTrigger.addEventListener("click", (e) => {
        let submenuContent = e.target.parentElement.querySelector(
  ...
  secondLevelMenuToggle() {
    let secondLevelMenus = this.mainContent
      .querySelector("ul")
      .querySelectorAll(":scope > li.has-submenu");
    secondLevelMenus.forEach((item) => {
      let allmenus = item
        .querySelector(".main-nav__submenu-content")
        .querySelectorAll(".down-arrow-icon");
      allmenus.forEach((item) => {
        // item.style.border="2px solid red";
        item.addEventListener("click", (e) => {
          let submenu = e.target.parentElement.querySelector(
            ".main-nav__submenu-content"
          );
...
let subpageFinder = document.querySelector(".subpage-content");
if (subpageFinder) {
...
// let landingPageClasstoHeader = document.querySelector(".landing-page");
// if (landingPageClasstoHeader) {
//   document.querySelector(".aegis-header-static-box").style.display = "none";
// }
...
/* Landing page call and whats app icons: Start */
let lfb = document.querySelector(".landing-page-floating-buttons");
if (lfb) {
  window.addEventListener("scroll", () => {
    if (window.scrollY > 200) {
      lfb.classList.add("landing-page-floating-buttons--active");
    } else {
      lfb.classList.remove("landing-page-floating-buttons--active");
    }
  });
}

/* Landing page call and whats app icons: End */

// Social Meida Button for Landing Page: Start
let smbFinding = document.querySelector(".landing-page");
if (smbFinding) {
  let smb = document.querySelector(".social-media");
  smb.classList.add("social-media-for-landing-page");
}
// Social Meida Button for Landing Page: End

function validateInput(input) {
  input.value = input.value.replace(/[^A-Za-z ]/g, ""); // Removes non-letter characters
}

let gfq = document.querySelector(".landing-page-form");
if (gfq) {
  let ssl = document.querySelector(".sidebar-services-listbox");
  // ssl.classList.add("with-get-free-quote");
}
