...
if(@$id != "")
{
	$sql = "SELECT * FROM services_brief where id=$id";  
	$result = $db->query($sql); 
	$row = $result->fetch_array();
	$imgPath="";
	if(@$row['image'] != "")
	{
		$imgPath=$adminPath . 'uploads/servicesbrief/' . @$row['image'];
	}
	if(@$row['icon_image1'] != "")
	{
		$icon_image1=$adminPath . 'uploads/servicesbrief/' . @$row['icon_image1'];
	}
	if(@$row['icon_image2'] != "")
	{
		$icon_image2=$adminPath . 'uploads/servicesbrief/' . @$row['icon_image2'];
	}
	if(@$row['icon_image3'] != "")
	{
		$icon_image3=$adminPath . 'uploads/servicesbrief/' . @$row['icon_image3'];
	}
}
?>
<div class="admin-main-section dashboard">
		<h1 class="admin-main-section__heading">Edit Services Brief</h1>
		
		<div class="admin-main-section__content">
		<div class="href_back"><a href="<?php echo $adminPath;?>servicesbrief/list.php" class="btn-default">Back</a></div>
					<div id="msg"></div>

						<!-- <h3>Add</h3> -->
						<form name="frmeditservicesbrief" id="frmeditservicesbrief" method="post" enctype="multipart/form-data">
						<input type="hidden" name="id" id="id" value="<?php echo $id; ?>" />
							<table style="padding: 0; margin: 0;">
								...
								<tr>	
									...
									<button class="form__submit btn1" type="submit" name="btnservicesbriefupdate" id="btnservicesbriefupdate" onclick="submitUpdateServicesBrief();">Submit</button>
									</td>
								</tr>
							</table>
