...
$subMenuList = $db->query($subMenuSql);
		...
		if(@$id != "")
		{
					$sql = "SELECT * FROM pages where id=$id and status=0";  
					$result = $db->query($sql); 
					$row = $result->fetch_array();
					$imgPath="";
					if(@$row['image'] != "")
					{
						$imgPath=$adminPath . 'uploads/page/' . @$row['image'];
					}
		}
?>
<div class="admin-main-section dashboard">
		<h1 class="admin-main-section__heading">Edit Sub Page </h1>
		...
		<div class="href_back"><a href="<?php echo $adminPath;?>page/list.php" class="btn-default">Back</a></div>
					...
					<div id="msg"></div>
					<form name="frmeditpage" id="frmeditpage" method="post" enctype="multipart/form-data">
									...
									<tr>
										...
										<tr>
											<td colspan="2" class="cls_btn"><input class="form__submit btn1" type="submit" onclick="submitEditPage();" value="Submit" name="btnPageAdd" id="btnPageAdd"></td>
										</tr>		
