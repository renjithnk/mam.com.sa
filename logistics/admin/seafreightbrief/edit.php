...
if(@$id != "")
{
	$sql = "SELECT * FROM seafreight_brief where id=$id and status=0";  
	$result = $db->query($sql); 
	if($result)
	{
		...
		if(@$row['image'] != "")
		{
			$imgPath=$adminPath . 'uploads/seafreightbrief/' . @$row['image'];
		}
		if(@$row['icon_image1'] != "")
		{
			$icon_image1=$adminPath . 'uploads/seafreightbrief/' . @$row['icon_image1'];
		}
	}
}
?>
<div class="admin-main-section dashboard">
		<h1 class="admin-main-section__heading">Edit Sea Freight</h1>
		
		<div class="admin-main-section__content">
		<div class="href_back"><a href="<?php echo $adminPath;?>seafreightbrief/list.php" class="btn-default">Back</a></div>
					<div id="msg"></div>

						<!-- <h3>Add</h3> -->
						<form name="frmeditseafreight" id="frmeditseafreight" method="post" enctype="multipart/form-data">
						<input type="hidden" name="id" id="id" value="<?php echo $id; ?>" />
							<table style="padding: 0; margin: 0;">
								...
								<tr>	
									...
									<button class="form__submit btn1" type="submit" name="btnseafreightadd" id="btnseafreightadd" onclick="submitUpdateSeaFreight();">Submit</button>
									</td>
								</tr>
							</table>
